#!/usr/bin/env python3
"""Generate branded Peter Lynch-style equity screen PDFs for RupeeMap.in."""
import os
from fpdf import FPDF
import common_fin  # shared quantitative engine

FONT = os.getenv("PERSONA_FONT", "/Library/Fonts/Arial Unicode.ttf")
OUT = os.path.join("/tmp", "peter-lynch_reports")
...
PERSONA = (
    "This report is written in the voice and discipline of Peter Lynch, the legendary manager of "
    "Fidelity's Magellan Fund (1977-1990), who compounded capital at roughly 29% a year. His core "
    "ideas, drawn from 'One Up on Wall Street' and 'Beating the Street', guide every line below:\n\n"
    "- Invest in what you understand. If you can't explain the business in a sentence, move on.\n"
    "- Growth at a reasonable price (GARP). A wonderful business is only a buy at the right price.\n"
    "- Know the kind of stock you own: a stalwart, a fast grower, a cyclical, a turnaround, or an asset play.\n"
    "- Do your homework on the numbers - earnings, debt, cash flow and valuation - then act with conviction.\n"
    "- The person who turns over the most rocks wins the game.\n\n"
    "These screens are a first pass - the kind of quick, skeptical look Lynch would take before deciding "
    "whether a company deserves a deeper dig. They are educational, not a recommendation to buy or sell."
)
...
class Report(FPDF):
    ...
    def header(self):
        ...
        if os.path.exists(LOGO_PATH):
            ...
            self.cell(95, 4, s("Peter Lynch Style Equity Screen"), ln=0)
            self.set_xy(192, 9)
            self.set_font("Arial", "", 8)
            self.set_text_color(*GREY)
    ...
    def cover(self):
        ...
        if os.path.exists(LOGO_PATH):
            ...
            self.set_text_color(*GREY)
            self.cell(0, 6, s("Equity Research  -  Peter Lynch Style Screen"), ln=1)
            self.set_xy(44, 26)
        else:
            self.set_xy(18, 16)
        ...
        self.set_text_color(*GREY)
        self.multi_cell(174, 5.5, s('"The person who turns over the most rocks wins the game."  -  Peter Lynch'))
        ...
        self.set_font("Arial", "", 8)
        self.set_text_color(*GREY)
        self.multi_cell(174, 5, s("RupeeMap.in (https://rupeemap.in/) applies a Peter Lynch-inspired, evidence-based "
    ...
    def persona_page(self):
        self.add_page()
        self.section_title("The Peter Lynch Persona")
        ...
        self.multi_cell(174, 5.2, s("Every report in this series follows the same discipline: route the request, "
                                    ...
                                    "above, and end with one of three calls - SKIP, WATCHLIST, or DEEP RESEARCH."))
    ...
    def body(self):
        ...
        for label, state, conv, ev in d["diagnostics"]:
            # line 1: label (navy) + state (teal, allowed to wrap) -> no horizontal overflow
            self.set_font("Arial", "B", 10)
            self.set_text_color(*NAVY)
            self.set_x(18)
            self.cell(50, 6, s(label), ln=0)
            self.set_text_color(*TEAL2)
            self.set_font("Arial", "B", 9.5)
            self.multi_cell(126, 6, s(state), ln=1, align="L")
            # line 2: conviction (its own line -> cannot collide with state)
            self.set_x(18)
            self.set_text_color(*GREY)
            self.set_font("Arial", "", 9)
            self.cell(0, 5, s("Conviction: " + conv), ln=1)
            # line 3: evidence (wrapped, full width)
            self.set_text_color(*DARK)
            self.set_font("Arial", "", 9.5)
            self.set_x(18)
        ...
        self.ln(1)
        self.section_title("4) Category Specific Check")
        self.set_font("Arial", "", 10)
        self.set_x(18)
