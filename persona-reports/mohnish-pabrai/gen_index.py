#!/usr/bin/env python3
"""Automated Mohnish Pabrai-style equity-screen report generator for NSE indices.

Fetches live financials via yfinance, runs the Mohnish Pabrai fatal-flaw framework,
...
def analyze(sym, name, industry, fetched):
    ...
    if peg is not None and peg > 2.5:
        ...
        val_watch = True
        val_parts.append(f"PEG {peg:.1f} is above 1.5")
    if val_flag:
        flaws.append(("Margin of safety", "FLAG", "Valuation leaves no margin of safety - " + "; ".join(val_parts) + ". Pabrai buys dollars for forty cents; this is priced for perfection."))
    elif val_watch:
        flaws.append(("Margin of safety", "WATCH", "Valuation is full-ish - " + "; ".join(val_parts) + ". Wait for a fatter pitch."))
    else:
        pe_s = f"trailing P/E {pe:.0f}" if pe is not None else "P/E n/a"
        flaws.append(("Margin of safety", "PASS", f"Valuation is cheap enough ({pe_s}); the price offers a real margin of safety - a dollar of value at a discount."))

    if is_financial:
        if de_pct is not None and de_pct > 300:
            flaws.append(("Leverage & safety", "WATCH", f"Leverage is high (~{de_pct:.0f}%), normal for a financial; judge via capital/asset quality, not D/E."))
        else:
            flaws.append(("Leverage & safety", "PASS", "Leverage is part of the normal financial model; assess solvency via capital and asset quality."))
    elif de_pct is None:
        flaws.append(("Leverage & safety", "WATCH", "Debt-to-equity could not be confirmed; verify the balance sheet before cloning the idea."))
    elif de_pct > 80:
        flaws.append(("Leverage & safety", "FLAG", f"Debt-to-equity is high at ~{de_pct:.0f}% - leverage that can break a good business in a downturn."))
    elif de_pct > 40:
        flaws.append(("Leverage & safety", "WATCH", f"Debt-to-equity is elevated at ~{de_pct:.0f}%; acceptable but watch versus cash generation."))
    else:
        flaws.append(("Leverage & safety", "PASS", f"Debt-to-equity is low at ~{de_pct:.0f}%; a sturdy, low-leverage balance sheet."))

    ret_parts = []
    rscore = 0
    if roe is not None and roe > 15:
        rscore += 1
        ret_parts.append(f"ROE ~{roe:.0f}%")
    if roic is not None and roic > 12:
        rscore += 1
        ret_parts.append(f"ROIC ~{roic:.0f}%")
    elif roic is not None and roic > 0:
        ret_parts.append(f"ROIC ~{roic:.0f}%")
    if rscore >= 2:
        flaws.append(("Returns on capital", "PASS", "High, durable returns on capital (" + ", ".join(ret_parts) + ") - a franchise that compounds."))
    elif rscore == 1:
        flaws.append(("Returns on capital", "WATCH", "Returns are mixed (" + ", ".join(ret_parts) + "); the franchise's quality is unproven."))
    else:
        flaws.append(("Returns on capital", "FLAG", "Returns on capital are low or unavailable (" + ", ".join(ret_parts) + "); little evidence of a moat."))
    ...
    if eps_g is None and ni_g is None:
        ...
        flaws.append(("Earnings power", "FLAG", f"Earnings are contracting ({earn_str}) - an economic engine that may be sputtering."))
    elif (eps_g is not None and eps_g < 0):
        ...
        flaws.append(("Earnings power", "PASS", f"Earnings are growing (latest {earn_str}); the business produces more owner earnings over time."))
    ...
    if summary and len(summary) > 40:
        flaws.append(("Understandability", "PASS", "The business is describable in a sentence - it sits inside the circle of competence."))
    else:
        flaws.append(("Understandability", "WATCH", "The business is hard to characterize from available data; demand a clearer story before cloning."))
