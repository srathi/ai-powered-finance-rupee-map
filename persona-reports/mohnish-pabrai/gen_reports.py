#!/usr/bin/env python3
"""Generate branded Mohnish Pabrai-style equity screen PDFs for RupeeMap.in."""
import os
from fpdf import FPDF
import common_fin  # shared quantitative engine

FONT = os.getenv("PERSONA_FONT", "/Library/Fonts/Arial Unicode.ttf")
OUT = os.path.join("/tmp", "mohnish-pabrai_reports")
...
PERSONA = (
"This report is written in the voice and discipline of Mohnish Pabrai, founder of Pabrai Investment Funds and author of The Dhandho Investor, who clones the proven ideas of great investors and insists on buying far below intrinsic value. His core ideas guide every line below:\n\n- Clone high-conviction ideas: why invent when you can copy the proven bets of superior investors?\n- Demand a huge margin of safety: buy a dollar of value for forty cents, so the odds of losing money collapse.\n- Concentrate in a few deep, low-risk / high-upside bets - 'heads I win, tails I do not lose much.'\n- Favour low P/E and P/B, high ROIC/ROE, little or no debt, and honest capital allocators.\n- Stick to understandable businesses with long runways; avoid complex, fraud-prone or glamour tech at rich multiples.\n\nThese screens are a first pass - the kind of quick, clone-and-verify look Pabrai would take before sizing a position. They are educational, not a recommendation to buy or sell."
)
...
class Report(FPDF):
    ...
    def header(self):
        ...
        if os.path.exists(LOGO_PATH):
            ...
            self.cell(95, 4, s("Mohnish Pabrai Style Equity Screen"), ln=0)
            self.set_xy(192, 9)
            self.set_font("Arial", "", 8)
            self.set_text_color(*GREY)
    ...
    def cover(self):
        ...
        if os.path.exists(LOGO_PATH):
            ...
            self.set_text_color(*GREY)
            self.cell(0, 6, s("Equity Research  -  Mohnish Pabrai Style Screen"), ln=1)
            self.set_xy(44, 26)
        else:
            self.set_xy(18, 16)
        ...
        self.set_text_color(*GREY)
        self.multi_cell(174, 5.5, s('"Heads I win; tails I do not lose much."  -  Mohnish Pabrai'))
        ...
        self.set_font("Arial", "", 8)
        self.set_text_color(*GREY)
        self.multi_cell(174, 5, s("RupeeMap.in (https://rupeemap.in/) applies a Mohnish Pabrai-inspired, evidence-based "
    ...
    def persona_page(self):
        self.add_page()
        self.section_title("The Mohnish Pabrai Persona")
