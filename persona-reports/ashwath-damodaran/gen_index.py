#!/usr/bin/env python3
"""Automated Aswath Damodaran-style equity-screen report generator for NSE indices.

Fetches live financials via yfinance, runs the Aswath Damodaran fatal-flaw framework,
...
def analyze(sym, name, industry, fetched):
    ...
    if val_flag:
        flaws.append(("Pricing vs value", "FLAG", "The multiple is hard to justify with cash flows - " + "; ".join(val_parts) + ". The story must be extraordinary to back this price."))
    elif val_watch:
        flaws.append(("Pricing vs value", "WATCH", "Valuation is full-ish - " + "; ".join(val_parts) + ". The narrative must clearly earn this price."))
    else:
        pe_s = f"trailing P/E {pe:.0f}" if pe is not None else "P/E n/a"
        flaws.append(("Pricing vs value", "PASS", f"Valuation is reasonable ({pe_s}); pricing and the intrinsic-value story can agree."))

    if (roe is not None and roe > 15) or (roic is not None and roic > 12):
        flaws.append(("Returns vs cost of capital", "PASS", f"Returns exceed the cost of capital (ROE ~{roe:.0f}%, ROIC ~{roic:.0f}%); value is being created." if (roe is not None and roic is not None) else "Returns are healthy; value is being created."))
    elif (roe is not None and roe > 0) or (roic is not None and roic > 0):
        flaws.append(("Returns vs cost of capital", "WATCH", f"Returns are thin (ROE ~{roe:.0f}%, ROIC ~{roic:.0f}%); barely above the cost of capital." if (roe is not None and roic is not None) else "Returns are thin; barely above the cost of capital."))
    else:
        flaws.append(("Returns vs cost of capital", "FLAG", "Returns are low or negative; the business destroys rather than creates value."))

    if (eps_g is not None and eps_g > 25) or (rev_g is not None and rev_g > 30):
        flaws.append(("Growth achievability", "WATCH", f"Implied growth is very high (revenue {rev_g:+.1f}%, EPS {eps_g:+.1f}%); is this achievable, or a fairy tale?" if (rev_g is not None and eps_g is not None) else "Implied growth is very high; is it achievable, or a fairy tale?"))
    elif (eps_g is not None and eps_g > 0) or (rev_g is not None and rev_g > 0):
        flaws.append(("Growth achievability", "PASS", f"Growth is positive and plausible (revenue {rev_g:+.1f}%, EPS {eps_g:+.1f}%)." if (rev_g is not None and eps_g is not None) else "Growth is positive and plausible."))
    else:
        flaws.append(("Growth achievability", "WATCH", "Growth is flat or negative; the narrative needs a credible turnaround story."))

    if margin is not None and margin > 0:
        flaws.append(("Pricing power", "PASS", f"Net margin ~{margin:.1f}% indicates some pricing power in the franchise."))
    elif margin is not None:
        flaws.append(("Pricing power", "WATCH", f"Net margin is thin/negative (~{margin:.1f}%); little evidence of pricing power."))
    else:
        flaws.append(("Pricing power", "WATCH", "Margin data unavailable; pricing power unconfirmed."))

    if is_financial:
        flaws.append(("Cash-flow backbone", "PASS", "Cash-flow conversion is not a meaningful test for a financial; judge via loan/book growth and asset quality."))
    elif fcf_latest is not None and ni_latest is not None and ni_latest != 0:
        conv = fcf_latest / ni_latest
        if conv < 0.2:
            flaws.append(("Cash-flow backbone", "FLAG", f"Free cash flow ({fmt_cr(fcf_latest)}) is well below net profit ({fmt_cr(ni_latest)}); the story is not backed by cash."))
        elif conv < 0.6:
            flaws.append(("Cash-flow backbone", "WATCH", f"Free cash flow ({fmt_cr(fcf_latest)}) converts at ~{conv*100:.0f}% of net profit ({fmt_cr(ni_latest)}); watch the gap between story and cash."))
        else:
            flaws.append(("Cash-flow backbone", "PASS", f"Free cash flow ({fmt_cr(fcf_latest)}) backs net profit ({fmt_cr(ni_latest)}) at ~{conv*100:.0f}%; the story is backed by cash."))
    else:
        flaws.append(("Cash-flow backbone", "WATCH", "Free-cash-flow conversion could not be verified from the statements."))

    if de_pct is None:
        flaws.append(("Balance sheet", "WATCH", "Debt-to-equity could not be confirmed; verify the balance sheet."))
    elif de_pct > 120:
        flaws.append(("Balance sheet", "WATCH", f"Debt-to-equity is high at ~{de_pct:.0f}%; a fragile balance sheet that can break the valuation."))
    else:
        flaws.append(("Balance sheet", "PASS", f"Debt-to-equity is ~{de_pct:.0f}%; a sturdy balance sheet underpins the narrative."))
