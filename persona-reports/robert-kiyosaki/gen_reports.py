#!/usr/bin/env python3
"""Generate branded Robert Kiyosaki-style equity screen PDFs for RupeeMap.in."""
import os
from fpdf import FPDF
import common_fin  # shared quantitative engine

FONT = os.getenv("PERSONA_FONT", "/Library/Fonts/Arial Unicode.ttf")
OUT = os.path.join("/tmp", "robert-kiyosaki_reports")
...
PERSONA = (
"This report is written in the voice and discipline of Robert Kiyosaki, author of Rich Dad Poor Dad and a champion of financial education. His cash-flow owner mindset guides every line below:\n\n- Treat every equity as a business asset: an asset puts money in your pocket, a liability takes it out.\n- Favour companies that generate real free cash flow and pay income (dividends) - true passive income.\n- Be wary of speculation and fragile balance sheets; price is only half the story, cash flow is the test.\n- Use other people's money wisely and aim for financial independence through income-producing assets.\n\nThese screens are a first pass - the kind of cash-flow-and-balance-sheet look Kiyosaki would take before calling a stock an asset. They are educational, not a recommendation to buy or sell."
)
...
class Report(FPDF):
    ...
    def header(self):
        ...
        if os.path.exists(LOGO_PATH):
            ...
            self.cell(95, 4, s("Robert Kiyosaki Style Equity Screen"), ln=0)
            self.set_xy(192, 9)
            self.set_font("Arial", "", 8)
            self.set_text_color(*GREY)
    ...
    def cover(self):
        ...
        if os.path.exists(LOGO_PATH):
            ...
            self.set_text_color(*GREY)
            self.cell(0, 6, s("Equity Research  -  Robert Kiyosaki Style Screen"), ln=1)
            self.set_xy(44, 26)
        else:
            self.set_xy(18, 16)
        ...
        self.set_text_color(*GREY)
        self.multi_cell(174, 5.5, s('"An asset puts money in my pocket. A liability takes money out of my pocket."  -  Robert Kiyosaki'))
        ...
        self.set_font("Arial", "", 8)
        self.set_text_color(*GREY)
        self.multi_cell(174, 5, s("RupeeMap.in (https://rupeemap.in/) applies a Robert Kiyosaki-inspired, evidence-based "
    ...
    def persona_page(self):
        self.add_page()
        self.section_title("The Robert Kiyosaki Persona")
