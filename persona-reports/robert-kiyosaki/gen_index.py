#!/usr/bin/env python3
"""Automated Robert Kiyosaki-style equity-screen report generator for NSE indices.

Fetches live financials via yfinance, runs the Robert Kiyosaki fatal-flaw framework,
...
def analyze(sym, name, industry, fetched):
    ...
    if fcf_latest is not None and ni_latest is not None and ni_latest != 0:
        ...
        if conv > 0.6:
            flaws.append(("Cash flow", "PASS", f"Free cash flow ({fmt_cr(fcf_latest)}) converts at ~{conv*100:.0f}% of net profit ({fmt_cr(ni_latest)}); this equity throws off real cash - a true asset."))
        elif conv > 0.2:
            flaws.append(("Cash flow", "WATCH", f"Free cash flow ({fmt_cr(fcf_latest)}) converts at ~{conv*100:.0f}% of net profit ({fmt_cr(ni_latest)}); cash generation is thin."))
        else:
            flaws.append(("Cash flow", "FLAG", f"Free cash flow ({fmt_cr(fcf_latest)}) is well below net profit ({fmt_cr(ni_latest)}); the business bleeds cash - a liability, not an asset."))
    elif fcf_latest is not None and fcf_latest > 0:
        flaws.append(("Cash flow", "PASS", f"Free cash flow is positive ({fmt_cr(fcf_latest)}); the business generates cash."))
    else:
        flaws.append(("Cash flow", "WATCH", "Free-cash-flow could not be verified; an asset must produce cash flow."))

    if dy is not None and dy >= 1:
        flaws.append(("Passive income", "PASS", f"Pays a dividend (yield ~{dy:.1f}%) - regular income in your pocket."))
    elif dy is not None and dy > 0:
        flaws.append(("Passive income", "WATCH", f"Dividend yield is thin (~{dy:.1f}%); little passive income from this holding."))
    else:
        flaws.append(("Passive income", "WATCH", "No meaningful dividend; the only return relies on price appreciation (speculation)."))

    if is_financial:
        if de_pct is not None and de_pct > 300:
            flaws.append(("Balance-sheet strength", "WATCH", f"Leverage is high (~{de_pct:.0f}%), normal for a financial; judge via capital/asset quality, not D/E."))
        else:
            flaws.append(("Balance-sheet strength", "PASS", "Leverage is part of the normal financial model; assess solvency via capital and asset quality."))
    elif de_pct is None:
        flaws.append(("Balance-sheet strength", "WATCH", "Debt-to-equity could not be confirmed; verify the balance sheet."))
    elif de_pct > 120:
        flaws.append(("Balance-sheet strength", "FLAG", f"Debt-to-equity is high at ~{de_pct:.0f}% - a fragile balance sheet that can wreck the asset."))
    elif de_pct > 60:
        flaws.append(("Balance-sheet strength", "WATCH", f"Debt-to-equity is elevated at ~{de_pct:.0f}%; acceptable but watch the balance sheet."))
    else:
        flaws.append(("Balance-sheet strength", "PASS", f"Debt-to-equity is conservative at ~{de_pct:.0f}%; a sturdy, owner-friendly balance sheet."))

    if (eps_g is not None and eps_g > 0) or (ni_g is not None and ni_g > 0) or (rev_g is not None and rev_g > 0):
        flaws.append(("Pricing power", "PASS", f"Top line and earnings are growing (revenue {rev_g:+.1f}%, EPS {eps_g:+.1f}%); the business can raise prices." if (rev_g is not None and eps_g is not None) else "The business shows growth; some pricing power."))
    elif eps_g is None and ni_g is None and rev_g is None:
        flaws.append(("Pricing power", "WATCH", "Growth could not be verified; pricing power unconfirmed."))
    else:
        flaws.append(("Pricing power", "WATCH", f"Growth is flat or negative (revenue {rev_g:+.1f}%, EPS {eps_g:+.1f}%); weak pricing power." if (rev_g is not None and eps_g is not None) else "Growth is flat or negative; weak pricing power."))
    ...
    if val_watch:
        flaws.append(("Speculation vs value", "WATCH", "Valuation is rich - " + "; ".join(val_parts) + ". A price that depends on someone paying more is speculation, not an asset."))
    else:
        pe_s = f"trailing P/E {pe:.0f}" if pe is not None else "P/E n/a"
        flaws.append(("Speculation vs value", "PASS", f"Valuation is reasonable ({pe_s}); you are buying a business, not a lottery ticket."))
