#!/usr/bin/env python3
"""Generate branded Howard Marks-style equity screen PDFs for RupeeMap.in."""
import os
from fpdf import FPDF
import common_fin  # shared quantitative engine

FONT = os.getenv("PERSONA_FONT", "/Library/Fonts/Arial Unicode.ttf")
OUT = os.path.join("/tmp", "howard-marks_reports")
...
PERSONA = (
"This report is written in the voice and discipline of Howard Marks, co-founder of Oaktree Capital and author of The Most Important Thing and Mastering the Market Cycle. His memos on risk, psychology and cycles guide every line below:\n\n- Apply second-level thinking: don't just ask if a company is good - ask what the consensus thinks and whether the price reflects it.\n- Be mindful of cycles and where we stand in them; extrapolating the recent past is among the most dangerous things an investor can do.\n- Demand a margin of safety and genuine skepticism of market euphoria; the riskiest moment is when others believe there is no risk.\n- Avoid following the herd and avoid excessive leverage; control risk rather than forecast the macro.\n\nThese screens are a first pass - the kind of risk-aware, cycle-conscious look Marks would take before committing capital. They are educational, not a recommendation to buy or sell."
)
...
class Report(FPDF):
    ...
    def header(self):
        ...
        if os.path.exists(LOGO_PATH):
            ...
            self.cell(95, 4, s("Howard Marks Style Equity Screen"), ln=0)
            self.set_xy(192, 9)
            self.set_font("Arial", "", 8)
            self.set_text_color(*GREY)
    ...
    def cover(self):
        ...
        if os.path.exists(LOGO_PATH):
            ...
            self.set_text_color(*GREY)
            self.cell(0, 6, s("Equity Research  -  Howard Marks Style Screen"), ln=1)
            self.set_xy(44, 26)
        else:
            self.set_xy(18, 16)
        ...
        self.set_text_color(*GREY)
        self.multi_cell(174, 5.5, s('"We may never know where we\'re going, but we ought to know where we are."  -  Howard Marks'))
        ...
        self.set_font("Arial", "", 8)
        self.set_text_color(*GREY)
        self.multi_cell(174, 5, s("RupeeMap.in (https://rupeemap.in/) applies a Howard Marks-inspired, evidence-based "
    ...
    def persona_page(self):
        self.add_page()
        self.section_title("The Howard Marks Persona")
