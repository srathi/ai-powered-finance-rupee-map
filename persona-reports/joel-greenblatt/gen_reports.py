#!/usr/bin/env python3
"""Generate branded Joel Greenblatt-style equity screen PDFs for RupeeMap.in."""
import os
from fpdf import FPDF
import common_fin  # shared quantitative engine

FONT = os.getenv("PERSONA_FONT", "/Library/Fonts/Arial Unicode.ttf")
OUT = os.path.join("/tmp", "joel-greenblatt_reports")
...
PERSONA = (
    "This report is written in the voice and discipline of Joel Greenblatt, who distilled decades of "
    "market-beating results into 'The Little Book That Beats the Market' and its Magic Formula. His "
    "simple, mechanical, evidence-backed method guides every line below:\n\n"
    "- Rank the universe by two things only: a high earnings yield (cheapness) and a high return on capital (quality).\n"
    "- Earnings yield = EBIT / Enterprise Value - how much pre-tax cash flow you get per dollar of the whole business.\n"
    "- Return on capital = EBIT / (net working capital + net fixed assets) - how efficiently the business turns tangible capital into profit.\n"
    "- Buy the stocks that score best on BOTH, hold for a year, and ignore the headlines. The formula has historically beaten the market.\n\n"
    "These screens are a first pass - the kind of quantitative, emotion-free look Greenblatt would take before "
    "letting a stock into the formula's basket. They are educational, not a recommendation to buy or sell."
)
...
class Report(FPDF):
    ...
    def header(self):
        ...
        if os.path.exists(LOGO_PATH):
            ...
            self.cell(95, 4, s("Joel Greenblatt Magic Formula Screen"), ln=0)
            self.set_xy(192, 9)
            self.set_font("Arial", "", 8)
            self.set_text_color(*GREY)
    ...
    def cover(self):
        ...
        if os.path.exists(LOGO_PATH):
            ...
            self.set_text_color(*GREY)
            self.cell(0, 6, s("Equity Research  -  Joel Greenblatt Magic Formula Screen"), ln=1)
            self.set_xy(44, 26)
        else:
            self.set_xy(18, 16)
        ...
        self.set_text_color(*GREY)
        self.multi_cell(174, 5.5, s('"Buy good companies at bargain prices and have the patience to hold them."  -  Joel Greenblatt'))
        ...
        self.set_font("Arial", "", 8)
        self.set_text_color(*GREY)
        self.multi_cell(174, 5, s("RupeeMap.in (https://rupeemap.in/) applies a Joel Greenblatt-inspired Magic Formula "
    ...
    def persona_page(self):
        self.add_page()
        self.section_title("The Joel Greenblatt Persona")
        ...
        self.set_text_color(*GREY)
        self.set_x(18)
        self.multi_cell(174, 5.2, s("Every report in this series follows the same discipline: rank the universe by "
                                     "earnings yield and return on capital, screen against the simple two-metric test "
                                     "above, and end with one of three calls - TOP RANK, WATCH, or AVOID."))
    ...
    def body(self):
        ...
        self.ln(1)
        self.section_title("4) Magic Formula Verdict")
        self.set_font("Arial", "", 10)
        self.set_x(18)
...
def main():
    # Rendering is driven by gen_index.py, which feeds computed data dicts into
    # Report(). Running this module standalone is not supported.
    print("Use gen_index.py to generate reports from a stock list CSV.")
