#!/usr/bin/env python3
"""Generate branded Benjamin Graham-style equity screen PDFs for RupeeMap.in."""
import os
from fpdf import FPDF

FONT = os.getenv("PERSONA_FONT", "/Library/Fonts/Arial Unicode.ttf")
OUT = os.path.join("/tmp", "benjamin-graham_reports")
...
PERSONA = (
    "This report is written in the voice and discipline of Benjamin Graham, the 'father of value "
    "investing' and author of 'The Intelligent Investor'. His defensively-minded rules, designed to "
    "protect the investor from their own errors and Mr. Market's moods, guide every line below:\n\n"
    "- Insist on a margin of safety: buy well below a conservative estimate of intrinsic value.\n"
    "- The Graham number - sqrt(22.5 x EPS x Book Value per share) - is a disciplined upper bound on what a defensive investor should pay.\n"
    "- Demand earnings stability: profits positive through most years, not a roller-coaster.\n"
    "- Require financial strength: a current ratio above ~2 and manageable debt.\n"
    "- Hunt for net-nets - stocks trading below net current asset value - the purest margin of safety.\n\n"
    "These screens are a first pass - the kind of quantitative, risk-first look Graham would take before "
    "letting any stock into a defensive portfolio. They are educational, not a recommendation to buy or sell."
)
...
class Report(FPDF):
    ...
    def header(self):
        ...
        if os.path.exists(LOGO_PATH):
            ...
            self.cell(95, 4, s("Benjamin Graham Style Equity Screen"), ln=0)
            self.set_xy(192, 9)
            self.set_font("Arial", "", 8)
            self.set_text_color(*GREY)
    ...
    def cover(self):
        ...
        if os.path.exists(LOGO_PATH):
            ...
            self.set_text_color(*GREY)
            self.cell(0, 6, s("Equity Research  -  Benjamin Graham Style Screen"), ln=1)
        else:
            self.set_xy(18, 16)
        ...
        self.set_text_color(*GREY)
        self.multi_cell(174, 5.5, s('"The intelligent investor is a realist who sells to optimists and buys from pessimists."  -  Benjamin Graham'))
        ...
        self.set_font("Arial", "", 8)
        self.set_text_color(*GREY)
        self.multi_cell(174, 5, s("RupeeMap.in (https://rupeemap.in/) applies a Benjamin Graham-inspired, evidence-based "
    ...
    def persona_page(self):
        self.add_page()
        self.section_title("The Benjamin Graham Persona")
        ...
        self.multi_cell(174, 5.2, s("Every report in this series follows the same discipline: route the request, "
                                    ...
                                    "above, and end with one of three calls - REJECT, WATCH, or DEFENSIVE BUY."))
    ...
    def body(self):
        ...
        self.ln(1)
        self.section_title("4) Deep-Value Verdict")
        self.set_font("Arial", "", 10)
        self.set_x(18)
