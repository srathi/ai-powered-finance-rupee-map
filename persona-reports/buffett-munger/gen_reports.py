...
OUT = os.path.join("/tmp", "buffett-munger_reports")
...
PERSONA = (
    ...
    "- Look for a durable economic moat: high, consistent returns on capital that competitors cannot easily erode.\n"
...
class Report(FPDF):
    ...
    def header(self):
        ...
        if os.path.exists(LOGO_PATH):
            ...
            self.cell(95, 4, s("Buffett & Munger Style Equity Screen"), ln=0)
            self.set_xy(192, 9)
            self.set_font("Arial", "", 8)
            self.set_text_color(*GREY)
    ...
    def cover(self):
        ...
        if os.path.exists(LOGO_PATH):
            ...
            self.set_text_color(*GREY)
            self.cell(0, 6, s("Equity Research  -  Buffett & Munger Style Screen"), ln=1)
        else:
            self.set_xy(18, 16)
        ...
        self.set_font("Arial", "", 8)
        self.set_text_color(*GREY)
        self.multi_cell(174, 5, s("RupeeMap.in (https://rupeemap.in/) applies a Buffett & Munger-inspired, evidence-based "
