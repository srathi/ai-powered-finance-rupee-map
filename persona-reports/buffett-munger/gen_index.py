#!/usr/bin/env python3
"""Automated Buffett & Munger-style equity-screen report generator for NSE indices.

Fetches live financials via yfinance, runs the Buffett & Munger fatal-flaw framework,
...
def analyze(sym, name, industry, fetched):
    ...
    # ---- Buffett dimensions ----
    comprehensible = bool(summary) and len(summary) > 40
    comp_label = "Understandable" if comprehensible else "Hard to characterize"
    comp_conv = "High" if comprehensible else "Medium"

    eps_series = eps[-5:] if len(eps) >= 5 else eps
    eps_up_years = sum(1 for v in eps_series if v is not None and v > 0)
    ...
    if eps_up_years >= 4:
        moat_strength += 1
    if moat_strength >= 4:
        moat_label, moat_conv = "Wide", "High"
    elif moat_strength >= 2:
        moat_label, moat_conv = "Narrow", "Medium"
    else:
        moat_label, moat_conv = "None evident", "Low"

    if (roe is not None and roe > 15) and (eps_g is not None and eps_g > 0):
        earn_label, earn_conv = "Strong & consistent", "High"
    elif (roe is not None and roe > 10) or (eps_g is not None and eps_g > 0):
        earn_label, earn_conv = "Moderate", "Medium"
    else:
        earn_label, earn_conv = "Weak", "Low"

    if is_financial:
        bs_label, bs_conv = "Leverage normal for model", "Medium"
    elif net_cash:
        bs_label, bs_conv = "Net cash; ample firepower", "High"
    ...
    if is_financial:
        own_label, own_conv = "Not a meaningful test (financial)", "Medium"
    elif fcf_latest is not None and ni_latest is not None and ni_latest != 0:
        oconv = fcf_latest / ni_latest
        if oconv > 0.8:
            own_label, own_conv = "Strong", "High"
        elif oconv > 0.5:
            own_label, own_conv = "Moderate", "Medium"
        else:
            own_label, own_conv = "Weak", "Low"
    else:
        own_label, own_conv = "Unconfirmed", "Medium"

    ey = (1.0 / pe) if (pe is not None and pe > 0) else None
    if pe is not None and pe < 20 and (peg is None or peg < 1.5):
        mos_label, mos_conv = "Adequate", "High"
    elif pe is not None and pe < 30:
        mos_label, mos_conv = "Thin", "Medium"
    else:
        mos_label, mos_conv = "None (priced for perfection)", "Low"
    ...
    if eps_g is None and ni_g is None:
        flaws.append(("Earnings power", "WATCH", "Earnings trend could not be verified; treat the business's profit record as unconfirmed."))
    elif (eps_g is not None and eps_g < -15) or (ni_g is not None and ni_g < -20):
        flaws.append(("Earnings power", "FLAG", f"Earnings are contracting ({earn_str}) - a business whose economic engine is sputtering."))
    elif (eps_g is not None and eps_g < 0) or (ni_g is not None and ni_g < 0):
        flaws.append(("Earnings power", "WATCH", f"Recent earnings growth is flat-to-negative ({earn_str}); a temporary hiccup or a fading franchise?"))
    else:
        flaws.append(("Earnings power", "PASS", f"Earnings are growing (latest {earn_str}); the business is producing more owner earnings over time."))

    if is_financial:
        if de_pct is not None and de_pct > 300:
            flaws.append(("Financial strength", "WATCH", f"Balance-sheet leverage is high (~{de_pct:.0f}%), normal for a financial business; judge via capital/asset-quality ratios, not D/E."))
        else:
            flaws.append(("Financial strength", "PASS", "Leverage is part of the normal financial model; assess solvency via regulatory capital and asset quality."))
    elif de_pct is None:
        flaws.append(("Financial strength", "WATCH", "Debt-to-equity could not be confirmed; verify the balance sheet before sizing a position."))
    elif de_pct > 120:
        flaws.append(("Financial strength", "FLAG", f"Debt-to-equity is high at ~{de_pct:.0f}% - a fragile balance sheet that can break a good business in a downturn."))
    elif de_pct > 60:
        flaws.append(("Financial strength", "WATCH", f"Debt-to-equity is elevated at ~{de_pct:.0f}%; acceptable but watch versus cash generation."))
    else:
        flaws.append(("Financial strength", "PASS", f"Debt-to-equity is conservative at ~{de_pct:.0f}%; the balance sheet can weather a storm."))
    ...
    if ey is not None and pe is not None and pe > 40:
        val_flag = True
        val_parts.append(f"earnings yield only ~{ey*100:.1f}%")
    if val_flag:
        flaws.append(("Margin of safety", "FLAG", "Valuation leaves no margin of safety - " + "; ".join(val_parts) + ". Price assumes perfection."))
    elif val_watch:
        ...
        ey_s = f", earnings yield ~{ey*100:.1f}%" if ey is not None else ""
        flaws.append(("Margin of safety", "PASS", f"Valuation is reasonable ({pe_s}{ey_s}); the price offers some margin of safety."))

    if price and hi and lo and hi > lo:
        off_high = (hi - price) / hi * 100.0
        if off_high > 40:
            flaws.append(("Mr. Market", "WATCH", f"Price is ~{off_high:.0f}% below its 52-week high; Mr. Market is fearful - opportunity or warning?"))
        else:
            flaws.append(("Mr. Market", "PASS", f"Price is within ~{off_high:.0f}% of its 52-week high; the tape is not signalling distress."))
    else:
        flaws.append(("Mr. Market", "WATCH", "Recent price trend could not be confirmed."))

    if is_financial:
        flaws.append(("Owner earnings", "PASS", "Cash-flow conversion is not a meaningful test for a financial business; judge via loan/book growth and asset quality."))
    elif fcf_latest is not None and ni_latest is not None and ni_latest != 0:
    ...
    if moat_label == "None evident":
        flaws.append(("Moat", "WATCH", "No durable competitive advantage is evident from returns and consistency; a commodity-like business."))
    else:
        flaws.append(("Moat", "PASS", f"Returns and consistency suggest a {moat_label.lower()} economic moat."))
    ...
    if moat_label == "Wide":
        drv.append("a wide economic moat")
    elif moat_label == "Narrow":
        drv.append("a narrow economic moat")
    ...
    tagline = (f"A {sector.lower()} business screened as {decision} "
               ...
               ("a disciplined, long-term compounder candidate." if decision == "BUY CANDIDATE"
                else "worth monitoring as the thesis or the price clears." if decision == "WATCH"
                else "a fatal-flaw check fails; better avoided for now."))

    diagnostics = [
        ("Circle of competence", comp_label, comp_conv,
         "Business " + ("we can explain in a sentence" if comprehensible else "is hard to characterize from available data") + "."),
        ("Economic moat", moat_label, moat_conv,
         (f"ROE ~{roe:.0f}%, net margin ~{margin:.0f}%, EPS positive in {eps_up_years} of recent years."
          if (roe is not None and margin is not None) else "Return and margin data unavailable.")),
        ("Earnings power", earn_label, earn_conv,
         (f"ROE ~{roe:.0f}%; latest EPS growth {eps_g:+.1f}%." if (roe is not None and eps_g is not None) else "Earnings data unavailable.")),
        ("Balance sheet & capital allocation", bs_label, bs_conv,
        ...
        ("Owner-earnings quality", own_label, own_conv,
         ("Free cash flow backs the net profit." if own_label in ("Strong", "Moderate")
          else "Free-cash-flow conversion is weak or unconfirmed.")),
        ("Margin of safety", mos_label, mos_conv,
         (f"Trailing P/E {pe:.0f}, PEG {peg:.1f}, earnings yield ~{ey*100:.1f}% vs ~6-7% risk-free."
          if (pe is not None and ey is not None and peg is not None) else
          f"Trailing P/E {pe:.0f}, earnings yield ~{ey*100:.1f}% vs ~6-7% risk-free." if (pe is not None and ey is not None) else
          "Valuation data unavailable.")),
    ]

    if (moat_label == "Wide") and (bs_label.startswith("Net cash") or (bs_label == "Conservative" and mos_label == "Adequate")):
        category = "A wonderful business at a fair price - the classic Buffett compounder: a wide moat, a sturdy balance sheet, and a price that leaves a margin of safety."
    elif moat_label == "Wide":
        category = "A great business, but the price leaves little margin of safety. The move is to wait for a better entry, not to force the purchase."
    elif moat_label == "Narrow":
        category = "A reasonably good business; the edge is real but thin. Returns depend more on the price paid and balance-sheet strength."
    else:
        category = "A commodity-like or weak business with no clear moat; only a very cheap price would make it interesting."
    ...
    decision_reason = (f"Screened as {decision}. " +
                       ("A fatal-flaw check fails (see flagged items), so it is set aside - price or quality is not there."
                        if decision == "AVOID" else
                        "Fundamentals are acceptable but one or more checks (moat, balance sheet, valuation or earnings quality) warrant monitoring before committing capital."
                        if decision == "WATCH" else
                        "Shows a durable franchise with acceptable valuation and a sound balance sheet; a candidate for deeper, owner-oriented due diligence."))

    revisit = ("Revisit if the flagged weakness (earnings, debt, cash flow, moat or valuation) clearly reverses."
