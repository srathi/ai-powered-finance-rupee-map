#!/usr/bin/env python3
"""Generate branded Raamdeo Agrawal-style equity screen PDFs for RupeeMap.in."""
import os
from fpdf import FPDF

FONT = os.getenv("PERSONA_FONT", "/Library/Fonts/Arial Unicode.ttf")
OUT = os.path.join("/tmp", "raamdeo-agarwal_reports")
...
PERSONA = (
'This report is written in the voice and discipline of Raamdeo Agrawal, co-founder of Motilal Oswal and author of the Wealth Creation Studies. His QGLP framework guides every line below:\n\n- Quality: invest in excellent businesses with ethical, shareholder-oriented management and a real moat.\n- Growth: seek consistent compounding of earnings, the engine of long-term wealth creation.\n- Longevity: favour franchises that can sustain quality and growth across decades and cycles.\n- Price: even a wonderful business must be bought at a reasonable valuation (watch the PEG) for a margin of safety.\n\nThese screens are a first pass - the kind of QGLP quality-and-growth look Agrawal would take before buying. They are educational, not a recommendation to buy or sell.'
)
...
class Report(FPDF):
    ...
    def header(self):
        ...
        if os.path.exists(LOGO_PATH):
            ...
            self.cell(95, 4, s("Raamdeo Agrawal Style Equity Screen"), ln=0)
            self.set_xy(192, 9)
            self.set_font("Arial", "", 8)
            self.set_text_color(*GREY)
    ...
    def cover(self):
        ...
        if os.path.exists(LOGO_PATH):
            ...
            self.set_text_color(*GREY)
            self.cell(0, 6, s("Equity Research  -  Raamdeo Agrawal Style Screen"), ln=1)
        else:
            self.set_xy(18, 16)
        ...
        self.set_text_color(*GREY)
        self.multi_cell(174, 5.5, s('"QGLP - Quality, Growth, Longevity, and Price, with price last."  -  Raamdeo Agrawal'))
        ...
        self.set_font("Arial", "", 8)
        self.set_text_color(*GREY)
        self.multi_cell(174, 5, s("RupeeMap.in (https://rupeemap.in/) applies a Raamdeo Agrawal-inspired, evidence-based "
    ...
    def persona_page(self):
        self.add_page()
        self.section_title("The Raamdeo Agrawal Persona")
