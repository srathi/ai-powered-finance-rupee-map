#!/usr/bin/env python3
"""Automated Raamdeo Agrawal-style equity-screen report generator for NSE indices.

Fetches live financials via yfinance, runs the Raamdeo Agrawal fatal-flaw framework,
...
def analyze(sym, name, industry, fetched):
    ...
    if (roe is not None and roe > 15) or (roic is not None and roic > 12):
        flaws.append(("Quality (Q)", "PASS", f"High-quality franchise (ROE ~{roe:.0f}%, ROIC ~{roic:.0f}%)." if (roe is not None and roic is not None) else "High-quality franchise with healthy returns."))
    elif (roe is not None and roe > 10) or (roic is not None and roic > 0):
        flaws.append(("Quality (Q)", "WATCH", f"Quality is moderate (ROE ~{roe:.0f}%, ROIC ~{roic:.0f}%); a decent but not exceptional business." if (roe is not None and roic is not None) else "Quality is moderate; a decent but not exceptional business."))
    else:
        flaws.append(("Quality (Q)", "FLAG", "Quality is weak or unclear (low/negative returns); fails the first QGLP test."))

    if (eps_g is not None and eps_g > 12) or (ni_g is not None and ni_g > 12):
        flaws.append(("Growth (G)", "PASS", f"Earnings are compounding well (EPS {eps_g:+.1f}%, net profit {ni_g:+.1f}%)." if (eps_g is not None and ni_g is not None) else "Earnings are compounding well."))
    elif (eps_g is not None and eps_g > 0) or (ni_g is not None and ni_g > 0):
        flaws.append(("Growth (G)", "WATCH", f"Growth is positive but modest (EPS {eps_g:+.1f}%, net profit {ni_g:+.1f}%); watch for a quality trap." if (eps_g is not None and ni_g is not None) else "Growth is positive but modest; watch for a quality trap."))
    else:
        flaws.append(("Growth (G)", "FLAG", f"Earnings are stalling or falling (EPS {eps_g:+.1f}%, net profit {ni_g:+.1f}%); no compounding engine." if (eps_g is not None and ni_g is not None) else "Earnings are stalling or falling; no compounding engine."))

    if (eps_g is not None and eps_g > 8) and (roe is not None and roe > 12):
        flaws.append(("Longevity (L)", "PASS", "Quality and growth both look durable - a franchise built to last."))
    elif (roe is not None and roe > 10) or (eps_g is not None and eps_g > 0):
        flaws.append(("Longevity (L)", "WATCH", "Quality or growth is only moderate; durability across cycles is unproven."))
    else:
        flaws.append(("Longevity (L)", "FLAG", "Neither quality nor growth looks durable; the moat is weak."))

    if peg is not None and peg > 2.5:
        flaws.append(("Price (P)", "FLAG", f"PEG {peg:.1f} is too rich - even a great business at the wrong price fails QGLP."))
    elif peg is not None and peg > 1.5:
        flaws.append(("Price (P)", "WATCH", f"PEG {peg:.1f} is above 1.5; reasonable but leaves a thinner margin of safety."))
    elif pe is not None and pe > 40:
        flaws.append(("Price (P)", "WATCH", f"Trailing P/E {pe:.0f} is high; the price paid may not leave a margin of safety."))
    else:
        pe_s = f"P/E {pe:.0f}" if pe is not None else "P/E n/a"
        peg_s = f", PEG {peg:.1f}" if peg is not None else ""
        flaws.append(("Price (P)", "PASS", f"Valuation is reasonable ({pe_s}{peg_s}); price comes last and it checks out."))
    ...
    if eps_g is None and ni_g is None:
        flaws.append(("Earnings power", "WATCH", "Earnings trend could not be verified; treat the profit record as unconfirmed."))
    elif (eps_g is not None and eps_g < -15) or (ni_g is not None and ni_g < -20):
        flaws.append(("Earnings power", "FLAG", f"Earnings are contracting ({earn_str}) - the compounding story is broken."))
    else:
        flaws.append(("Earnings power", "PASS", f"Earnings are growing (latest {earn_str}); the engine of wealth creation is running."))
